...
var Service = module.exports = function(accessService) {
...
internal.findPersonsInConcepts = function(topics, uConcepts) {
	...
	for (i = 0; i < uConcepts.length; i++) {
		...
		for (var j = personKeys.length - 1; j >= 0; j--) {
			...
			if (core.text.endsWith(person.name.toLowerCase(), ' ' + concept.value.toLowerCase()) ||
				core.text.endsWith(person.name.toLowerCase(), ' ' + concept.atonic.toLowerCase())) {
				uConcepts.splice(i, 1);
				i--;
